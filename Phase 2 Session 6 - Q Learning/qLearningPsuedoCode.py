...
class QLearningAgent(ReinforcementAgent):
    ...
    def __init__(self, **args):
        ...
        "---------------"
        # initialise q values with empty dictionary
    ...
    def getQValue(self, state, action):
        ...
        "------------"
        # if state & action pair present in qval dictionary
            # return qval of that particular state and action pair
        # else
            # return 0.0 as qvalue

    def setQValue(self, state, action, value):
        self.qvalues[(state, action)] = value

        "------------"
        # setting qval for particular state & action particular
    ...
    def computeValueFromQValues(self, state):
        ...
        "-------"
        # For a state, from all possible actions :
            # Get qval for that state, action pair and insert in qval array
        # if qval array empty (i.e no possible action) : return 0.0
        # return max qval from all possible actions for that state
    ...
    def computeActionFromQValues(self, state):
        ...
        "--------"
        # Get best q-value from state (i.e. call computeValueFromQValues method)
        # From all possible actions in a state we chose and store that action in array
        # where the qvalue for that action, state pair matches with the best qvalue computed for that state in above step
        # We get best_actions for the nextState
        # if best_action array is empty : return None (i.e. no possible action in last state)
        # else return random action from the best actions
    ...
    def getAction(self, state):
        ...
        "--------"
        # Get all possible actions for a nextState
        # initialise action with None for start
        # use flipCoin method with epsilon for unbaisedness to assign action
        # if heads: action = random action from all possible actions for a state
        # else: action = Get best action from state (i.e. call computeActionFromQValues method)
        # return action
    def update(self, state, action, nextState, reward):
        ...
        # initialise learning rate assigned as alpha
        # Compute qval for the current state, action
        # Compute best q-val for the next state
        # Calculate a new qval taking the temporal difference into account
        # newqval = (1-alpha) * qvalue + alpha * (reward + disc * next_value)
        # Set the new calculated qvalue for the current state, action pair
